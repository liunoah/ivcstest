...
def client(user):
    ...
    # driver.find_elements(By.TAG_NAME, "li")[6].click()
...
def manage():
    print("manage start")
    cmd = "start C:/Users/root/Desktop/debug1"
    os.popen(cmd)
    # 打开新的标签页
    ch_option = webdriver.ChromeOptions()
    ch_option.add_experimental_option("debuggerAddress", "127.0.0.1:9225")
    driver = webdriver.Chrome(options=ch_option)
    sleep(2)
    js = "window.open('" + agent_url + "')"
    driver.execute_script(js)
...
def mian():
    ...
    _thread.start_new_thread(agent, (user1,))
    sleep(2)
